...
# def classes.Car.to_dict_car(obj):
#     '''
...
#     return data
#
# def classes.Car.from_dict_car(data):
#     '''
...
car_str_json = json.dumps(test_car, default=classes.Car.to_dict_car)
print(car_str_json)

# Create Car object from string:

car_from_string = classes.Car.from_dict_car(json.loads(car_str_json))
...
with open("car.json", "w") as file:
    json.dump(test_car, file, default=classes.Car.to_dict_car)

# Create Car object from file:
with open("car.json") as file:
    car_from_file = classes.Car.from_dict_car(json.load(file))
...
#             "owner": obj.owner}
#     if obj.cars:
#         serialized_cars = [json.dumps(car, default=classes.Car.to_dict_car) for car in obj.cars]
...
#     if data["cars"]:
#         restored_cars = [json.loads(car, object_hook=classes.Car.from_dict_car) for car in data["cars"]]
...
garage_str_json = json.dumps(test_garage, default=classes.Garage.to_dict_garage)
print(garage_str_json)

# Create garage object from string:

garage_from_string = classes.Garage.from_dict_garage(json.loads(garage_str_json))
...
with open("garage.json", "w") as file:
    json.dump(test_garage, file, default=classes.Garage.to_dict_garage)

# Create Garage object from file:
with open("garage.json") as file:
    garage_from_file = classes.Garage.from_dict_garage(json.load(file))
...
cesar_str_json = json.dumps(test_cesar, default=classes.Cesar.to_dict_cesar)
print(cesar_str_json)

# Create cesar object from string:

cesar_from_string = classes.Cesar.from_dict_cesar(json.loads(cesar_str_json))
...
with open("cesar.json", "w") as file:
    json.dump(test_cesar, file, default=classes.Cesar.to_dict_cesar)

# Create Cesar object from file:
with open("cesar.json") as file:
    cesar_from_file = classes.Cesar.from_dict_cesar(json.load(file))
...
# Conversion test_car to string:
car_str_yaml = yaml.dump(classes.Car.to_dict_car(test_car))
print(car_str_yaml)

# # Create Car object from string:

car_from_string = classes.Car.from_dict_car(yaml.load(car_str_yaml, Loader=yaml.Loader))
print(f"Type of restored Car: {type(car_from_string)}")
print(car_from_string)
...
with open("car.yaml", "w") as file:
    yaml.dump(classes.Car.to_dict_car(test_car), file)

# # Create Car object from file:
with open("car.yaml") as file:
    car_from_file = classes.Car.from_dict_car(yaml.load(file, Loader=yaml.Loader))
print(f"Type of restored Car: {type(car_from_file)}")
print(car_from_file)
